...
def trimal(input):
    print("NOW TRIMING :", input, "\n")
    file_name = directory + os.sep + input
    # popen = subprocess.Popen(['trimal', "-in", file_name, "-out",file_name + trimed_extension, "-gappyout"])  # ./trimal     -gappyout
    popen = subprocess.Popen(['trimal', "-in", file_name, "-out",file_name + trimed_extension, "-gt", "0.11","-htmlout", file_name + ".html"])  # ./trimal  -gt 0.11
    popen.wait()
    popen = subprocess.Popen(['pwd'])  # ./trimal
    popen.wait()
...
options (listed below) default to 'None' if omitted
    ...
    pool = Pool()
    list_of_files = [file for file in os.listdir(directory) if file.lower().endswith(aligned_extension)]
    pool.map(trimal, list_of_files)

    result = [file for file in os.listdir(directory) if file.lower().endswith(trimed_extension)]
    if isTabular(inputFile):
